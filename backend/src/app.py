...
class EUAIActQA:
    ...
    def get_db_connection(self):
        """
        Establishes a connection to the main database containing document chunks.

        Returns:
            psycopg2.connection: A live connection object to the main PostgreSQL database.
        """
    ...
    def get_logs_db_connection(self):
        """
        Establishes a connection to the logs database where queries and answers are stored.

        Returns:
            psycopg2.connection: A live connection object to the logs PostgreSQL database.
        """
        return psycopg2.connect(
    ...
    def log_query(self, query: str, relevant_chunks: List[Dict], answer: str):
        """
        Logs the question, answer, and the top retrieved document chunks into the logs database.

        Args:
            query (str): The user's input question.
            relevant_chunks (List[Dict]): The top-k relevant document chunks retrieved from the DB.
            answer (str): The answer generated by the LLM.

        Notes:
            - Automatically creates the `query_logs` table if it doesn't exist.
    ...
    def get_relevant_chunks(self, query: str, k: int = 3) -> List[Dict]:
        """
        Retrieves the most relevant document chunks from the main database using vector similarity.

        Args:
            query (str): The user's question to embed and search against.
            k (int): The number of top chunks to retrieve. Defaults to 3.
    ...
    def generate_answer(self, query: str, relevant_chunks: List[Dict]) -> str:
        ...
        Args:
            query (str): The user's input question.
            relevant_chunks (List[Dict]): The retrieved context chunks used to generate the answer.
...
if __name__ == "__main__":
    ...
    qa.log_query("What is the AI Act?", chunks, "It " \
    "regulates AI use in the EU.")
